...
However, setting philosophical orientations aside, nothing prevents us from evaluting the frequentist properties of a posterior mean.
...
1. Among the remaining unbiased estimators, select the one with the smallest variance. (The variance equls the MSE for unbiased estimators.)
...
The ML estimator $\hat{\lambda} = \frac{1}{\text{avg}(x)}$ of $\lambda$ for the exponential distribution is consistent. Comment on the relevance for Exercise \@ref{exrml-govt-duration}.
```
...
Comment on the ML estimator $\hat{\lambda} = \frac{1}{\text{avg}(x)}$ of $\lambda$ for the exponential distribution and the ML estimator $\hat{\mu} = \frac{1}{\hat{\lambda}}$ of the $\mu = \frac{1}{\lambda}$. Comment on the relevance for Exercise \@ref{exrml-govt-duration}.
```
